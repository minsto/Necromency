...
public class EntityIsaacHead extends Monster  {

    public EntityIsaacHead(EntityType<? extends Monster> type, Level level) {
        super(type, level);
    }
    ...
    public void die(DamageSource src) {
        ...
        if (!this.level().isClientSide() && this.level() instanceof ServerLevel level) {
            ItemStack stack = new ItemStack(ModItems.ISAACS_HEAD.get(), 1);

            ItemEntity drop = new ItemEntity(level, this.getX(), this.getY() + 0.5, this.getZ(), stack);
